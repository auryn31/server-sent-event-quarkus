...
class RestEndpointTest {
    ...
    @Test
    fun `there are 10 cars as response from api as stream`() {
        val url = URL("http://127.0.0.1:8080/cars")
        val con = url.openConnection()
        con.setRequestProperty("Accept", MediaType.APPLICATION_STREAM_JSON_VALUE)
    ...
    @Test
    fun `there are 10 cars as response from api as list`() {
        val url = URL("http://127.0.0.1:8080/cars")
        val con = url.openConnection()
